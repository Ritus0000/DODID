...
/* Показ скролл-полоски только во время движения */
let hideTimer=null;
...
function render(){
  ...
  tasks.forEach((t,i)=>{
    ...
    circle.addEventListener('click',()=>{
      ...
      // ВАЖНО: двигаем ровно <li> внутри #tasks — плюс в доке не трогаем
      const ul = document.getElementById('tasks');
      if (t.done) ul.appendChild(li);
    ...
    text.addEventListener('keydown',(e)=>{
      ...
      if((e.key==='Backspace'||e.key==='Delete') && val===''){
        e.preventDefault();
        tasks.splice(i,1);
        if(tasks.length===0) tasks.push({text:'',done:false});
        save(); render();
      }
    });
...
function addTask(){
  ...
  if(last){
    const c=last.querySelector('.circle');
    const tx=last.querySelector('.task-text');
...
/* Снятие фокуса по тапу вне текста */
...
/* Перерисовка зачёркивания при ресайзе */
